...
def make_whatsit(id: int = 123, name: str = "Test whatsit", purpose: str = "Test purpose") -> Whatsit:
    return Whatsit(whatsit_id=id, name=name, purpose=purpose)


class TestWhatsitResource:
    @patch.object(
        WhatsitService, "get_all", lambda: [make_whatsit(123, name="Test Whatsit 1"), make_whatsit(456, name="Test Whatsit 2")]
    )
    def test_get(self, client: FlaskClient):  # noqa
        with client:
            results = client.get(f"/api/{BASE_ROUTE}/whatsit", follow_redirects=True).get_json()
            expected = (
                WhatsitSchema(many=True)
                .dump([make_whatsit(123, name="Test Whatsit 1"), make_whatsit(456, name="Test Whatsit 2")])
                .data
            )
            for r in results:
                assert r in expected

    @patch.object(WhatsitService, "create", lambda create_request: Whatsit(**create_request))
    def test_post(self, client: FlaskClient):  # noqa
        with client:
            ...
            expected = WhatsitSchema().dump(Whatsit(name=payload["name"], purpose=payload["purpose"])).data
            assert result == expected


def fake_update(whatsit: Whatsit, changes: WhatsitInterface) -> Whatsit:
    # To fake an update, just return a new object
    updated_Whatsit = Whatsit(whatsit_id=whatsit.whatsit_id, name=changes["name"], purpose=changes["purpose"])
...
class TestWhatsitIdResource:
    ...
    def test_put(self, client: FlaskClient):  # noqa
        with client:
            result = client.put(f"/api/{BASE_ROUTE}/whatsit/123", json={"name": "New Whatsit", "purpose": "New purpose"}).get_json()
            expected = WhatsitSchema().dump(Whatsit(whatsit_id=123, name="New Whatsit", purpose="New purpose")).data
            assert result == expected
